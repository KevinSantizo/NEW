...
import BottomNavigation from '@/components/BottomNavigation'
  ...
  export default {
    ...
    methods: {
      getAll(){ 
      const path = 'http://172.20.10.2:8000/sport/reservations/'
      //const path = 'http://192.168.88.222:8000/sport/reservations/'
      axios.get(path).then((response) => {
        this.reservations = response.data
        console.log(this.reservations);
        
        return axios.get('http://172.20.10.2:8000/sport/companies/')
        //return axios.get('http://192.168.88.222:8000/sport/companies/');
        }).then((response) => {
          this.companies = response.data
          console.log(this.companies);
          
          return axios.get('http://172.20.10.2:8000/sport/fields/')
        }).then((response) => {
